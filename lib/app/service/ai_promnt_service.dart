...
class AIPromptService {
  ...
  // Expanded dictionary for spell-checking
  ...
  // Mock Bible data with support for multiple translations
    ...
    for (String word in words) {
      ...
      if (cleanWord.isNotEmpty) {
        if (RegExp(r'^[A-Z][a-z]*$').hasMatch(cleanWord)) {
          corrected = StringExtension(corrected).capitalize();
  ...
  String _correctWord(String word, bool isReligiousContext) {
    ...
    if (isReligiousContext) {
      threshold = math.min(threshold + 1, 5); // More lenient for religious terms
    }
  ...
  /// Checks if the prompt is likely religious
  ...
  String? checkCache(String prompt) {
  ...
  void cacheResponse(String prompt, String response) {
    _verseCache[prompt] = response;
  }

  /// Adds to session history
  void addToSessionHistory(String prompt, String response) {
    _sessionHistory.add({'prompt': prompt, 'response': response});
  }

  /// Checks if session history exists
  bool hasSessionHistory() {
    return _sessionHistory.isNotEmpty;
  }

  /// Gets the last prompt from session history
  String getLastPrompt() {
    return _sessionHistory.last['prompt']!;
  }

  /// Checks if the prompt is a "more" request
  bool isMorePrompt(String prompt) {
    final moreKeywords = [
      'more',
      'tell me more',
      'continue',
    ...
    final normalizedPrompt = prompt.toLowerCase().replaceAll(RegExp(r'[^\w\s]'), '').trim();
    return moreKeywords.any((kw) => normalizedPrompt == kw.replaceAll(RegExp(r'[^\w\s]'), ''));
  }

  /// Checks mock Bible data for exact verse matches
  String? checkBibleData(String prompt) {
    final cleanPrompt = prompt.toLowerCase().replaceAll(RegExp(r'[^\w\s]'), '');
...
''';
  ...
  String? checkAdditionalBibleData(String lastPrompt) {
...
''';
  ...
  Future<bool> isGreetingPrompt(String prompt) async {
  ...
  Future<bool> isReligiousPrompt(String prompt) async {
    final religiousKeywords = _dictionary
        .where(
          (word) => [
        'jesus', 'god', 'bible', 'christ', 'holy', 'spirit', 'faith', 'prayer', 'salvation', 'sin', 'grace', 'mercy',
        'forgiveness', 'heaven', 'hell', 'love', 'lord', 'scripture', 'gospel', 'messiah', 'resurrection', 'baptism',
        'parable', 'prophet', 'church', 'testament', 'covenant', 'miracle', 'sermon', 'psalm', 'proverb',
    ...
    final nonReligiousKeywords = [
      'weather', 'stock', 'recipe', 'sports', 'politics', 'movie', 'game', 'technology', 'news', 'science',
      'fashion', 'cook', 'travel', 'celebrity', 'finance', 'music', 'entertainment', 'health', 'fitness',
      'business', 'shopping', 'gadget', 'app', 'device', 'crypto', 'instagram', 'tiktok', 'facebook',
      'youtube', 'twitter', 'social', 'trend', 'meme', 'viral', 'podcast', 'streaming', 'netflix', 'spotify',
      'school', 'university', 'college', 'exam', 'homework', 'assignment', 'career', 'job', 'office',
      'interview', 'resume', 'startup', 'restaurant', 'cafe', 'bar', 'drink', 'coffee', 'tea', 'dessert',
      'lunch', 'dinner', 'breakfast', 'car', 'bike', 'flight', 'hotel', 'vacation', 'tour', 'festival',
